...
class MemoUpdateAPITest(BaseAPITest):
    def test_update_memo_should_success_with_valid_data_and_user(self):
        response = self.client.patch(f'/account-books/memos/{self.memo_id}', self.update_data, **self.headers)
    ...
    def test_update_memo_should_fail_with_not_authenticated_user(self):
        response = self.client.patch(f'/account-books/memos/{self.memo_id}', self.update_data)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_update_memo_should_fail_with_not_exists_memo(self):
        response = self.client.patch(f'/account-books/memos/{self.memo_id + 100}', self.update_data, **self.headers)
    ...
    def test_update_memo_should_fail_not_own_memo(self):
        ...
        response = self.client.patch(f'/account-books/memos/{self.memo_id}', self.update_data, **headers)
