...
class MemoGetAPITest(APITestCase):
    ...
    def test_get_memo_should_success_with_authenticated_user_and_exists_memo(self):
        response = self.client.get(f'/account_books/memos/{self.memo_id}', **self.headers)
    ...
    def test_get_memo_should_fail_with_not_exists_memo(self):
        response = self.client.get(f'/account_books/memos/{self.memo_id + 10}', **self.headers)
    ...
    def test_get_all_memos_should_success_with_authenticated_user(self):
        response = self.client.get(f'/account_books/memos', **self.headers)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 2)

    def test_get_all_memos_should_success_with_order_by_spend_price(self):
        response = self.client.get(f'/account_books/memos?order_by=spend_price', **self.headers)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_all_memos_should_fail_with_invalid_order_option(self):
        response = self.client.get(f'/account_books/memos?order_by=wrong_option', **self.headers)
